...
class OptionsScene(SceneBase):
    def __init__(self, app):
        super().__init__(app)
        self.title_text = "SELECT BOARD SIZE"
        ...
        # Lấy danh sách presets từ settings.py
        presets = list(BOARD_PRESETS.items())
        
        # Tạo nút cho từng kích thước
        for i, (name, (w, h)) in enumerate(presets):
            rect = pygame.Rect(cx - btn_w // 2, cy + i * spacing, btn_w, btn_h)
            # Dùng lambda với tham số mặc định để giữ giá trị w, h đúng cho từng vòng lặp
            btn = Button(rect, name, 
                         lambda w=w, h=h: self.apply_size(w, h), 
                         font_size=18)
            self.buttons.append(btn)

        # Nút Back (Quay lại)
        back_y = cy + len(presets) * spacing + 20
        self.buttons.append(Button(
            pygame.Rect(cx - btn_w // 2, back_y, btn_w, btn_h),
            "Back",
    ...
    def apply_size(self, w, h):
        # Gọi hàm update bên app.py để đổi kích thước cửa sổ
        if hasattr(self.app, 'update_grid_size'):
            self.app.update_grid_size(w, h)
    ...
    def draw(self, screen):
        ...
        # Vẽ tiêu đề
        title = self.font_title.render("OPTIONS", True, (120, 255, 120))
        # Scale chữ nhỏ lại chút vì tiêu đề dài
        if title.get_width() > self.cfg.width - 40:
             scale_factor = (self.cfg.width - 40) / title.get_width()
             title = pygame.transform.rotozoom(title, 0, scale_factor)
        else:
             title = pygame.transform.rotozoom(title, 0, 0.6)

        screen.blit(title, title.get_rect(center=(self.cfg.width // 2, 60)))
        
        msg = self.font_msg.render("Select Board Size:", True, (200, 200, 200))
        screen.blit(msg, msg.get_rect(center=(self.cfg.width // 2, 100)))
    ...
    def __init__(self, app):
        ...
        self.buttons = [
            ...
            Button(pygame.Rect(cx - btn_w//2, cy + spacing*2, btn_w, btn_h), "Options", 
                   lambda: self.app.change_scene(OptionsScene(self.app)), font_size_btn),
